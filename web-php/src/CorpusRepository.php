...
final class CorpusRepository
{
    ...
    public function __construct(?string $dataDir = null, ?string $cacheDir = null)
    {
        $this->dataDir = $dataDir
            ?? ($_ENV['CORPUS_DATA_DIR'] ?? dirname(__DIR__) . '/../web-angular/src/assets/data');
        $this->cacheDir = $cacheDir ?? dirname(__DIR__) . '/storage/cache';
    }

    /**
     * @return array{
     *   pairs: array<int, array{key:string, es:string, gl:string, alignmentCount:int, searchBlob:string}>,
     *   alignments: array<int, array{es:string, gl:string, tense:string, person:string, pairEs:string, pairGl:string, pairKey:string, searchBlob:string}>,
     *   tenseOptions: array<int, string>,
     *   personOptions: array<int, string>,
     *   missingAlignments: bool
     * }
     */
    public function getDataset(): array
    {
        $pairsPath = $this->dataDir . '/verbos_relacionados.tsv';
        $alignmentsPath = $this->dataDir . '/alineaciones_completas.tsv';
        ...
        $cacheKey = md5(json_encode([
            $pairsPath,
            is_file($pairsPath) ? filemtime($pairsPath) : null,
            is_file($pairsPath) ? filesize($pairsPath) : null,
            $alignmentsPath,
            is_file($alignmentsPath) ? filemtime($alignmentsPath) : null,
            is_file($alignmentsPath) ? filesize($alignmentsPath) : null,
        ], JSON_THROW_ON_ERROR));

        $cacheFile = $this->cacheDir . '/corpus-' . $cacheKey . '.cache';
            ...
            while (($line = fgets($handle)) !== false) {
                ...
                $alignments[] = [
                    ...
                    'searchBlob' => $this->normalizeText(
                        implode(' ', [$formEs, $formGl, $tense, $person, $currentPairEs, $currentPairGl])
                    ),
                ];

                if (!isset($pairAggregates[$pairKey])) {
                    $pairAggregates[$pairKey] = [
                        'count' => 0,
                        'parts' => [],
                    ];
                }
